...
poi_dict = {
    ...
    "Hotel": [
        200,
        "tourism.hostel,tourism.hotel,tourism.motel,tourism.apartment,tourism.guest_house",
    ],
...
def extract_nodes(pbf_file, poi):
    ...
    process = subprocess.Popen(
        osmosis_command, stdout=subprocess.PIPE, stderr=subprocess.PIPE, shell=True
    )
...
def create_gpi():
    ...
    process = subprocess.Popen(
        gpsbabel_command, stdout=subprocess.PIPE, stderr=subprocess.PIPE, shell=True
    )
    output, error = process.communicate()
